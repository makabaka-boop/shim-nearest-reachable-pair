...
某行目标不可达时，试配人员可在该行输入 **0～1000 μm 的整数容差**，
查询目标上下容差区间内是否存在真正可取用的 A、B 两片垫片。选择顺序：

1. 可达总厚度与目标差值的绝对值最小；
2. 差值并列时取**较小的总厚度**（即优先低于目标的一侧）；
3. 仍并列（同一总厚度有多种组合）时取**较小的 A 级规格**。

返回内容为 **A 规格、B 规格和有符号偏差**（`A+B − target`，负数表示低于目标）；
容差区间内无任何可达总厚度时显示“容差内无可达规格”。

实现要点：

- 邻近查询**复用当前有效输入构建的同一份位集**，由 Worker 在位集命中位中
  提取实际见证（最低/最高置位，取决于内部是否交换两级），
  **不枚举 A × B 数对**；内部为性能交换两级时，输出仍保持原 A/B 身份。
- **非法容差**（空、非整数、越界等）不会撤销任何东西：保留该行
  上一次有效邻近查询的结果，仅显示内联提示。
- **替换输入**（重新提交任何文本）立即撤销全部旧见证并递增输入令牌；
  新输入校验失败（INVALID_INPUT）时旧见证同样不再显示。
- 迟到的旧 Worker 响应（令牌不符、请求已被取代或行已不存在）一律丢弃，
  旧见证不能重新出现。
- 原批量可达结果的内容与顺序保持不变。
...
- **邻近可达规格**：独立实现的朴素参考（枚举 + 同序选择规则）与位集求解器
  在**小规模笛卡尔积**（多组规格 × 目标 × 容差）上逐一比对，覆盖选择顺序
  （最小差值 → 较小总厚度 → 较小 A 级规格）、重复规格、零值、0/400000 边界、
  容差 0 与 1000、内部交换两级时 A/B 身份保持；容差解析的合法/非法样例。
- **界面状态（jsdom）**：纯状态机测试覆盖换稿撤销、迟到响应丢弃、非法容差
  保留上次结果；App 级测试用内存假 Worker（跑真实求解逻辑）覆盖批量结果
  不变、邻近查询端到端、换稿与校验失败后的界面状态。
...
src/
  main.tsx                 # React 入口
  App.tsx                  # 单页界面（粘贴 JSON、状态、结果区、邻近查询接线）
  styles.css               # 页面样式
  solver.worker.ts         # Web Worker：批量求解 + 邻近查询（缓存当前有效输入位集）
  components/
    VirtualResults.tsx     # 结果虚拟列表（按原序，支持 10 万行，含邻近查询列）
    nearbyState.ts         # 邻近查询的纯 UI 状态机（撤销/迟到丢弃规则）
  lib/
    validation.ts          # JSON 解析与边界校验（INVALID_INPUT）+ 容差解析
    solver.ts              # BigInt 位集精确求解器 + 最近可达见证提取
  solver.test.ts           # 朴素/稠密/稀疏精确性测试
  validation.test.ts       # 输入校验测试
  nearby.test.ts           # 邻近可达规格：笛卡尔积朴素核对 + 边界/选择顺序
  App.test.tsx             # 界面测试（jsdom + 内存假 Worker）
  components/nearbyState.test.ts # 邻近查询状态机测试
scripts/
